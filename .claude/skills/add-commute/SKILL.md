...
# Add Commute / Travel Time Lookup

This skill adds live traffic-based travel time lookups using the Waze routing API (no API key required).

## Step 1: Create the commute plugin

```bash
mkdir -p plugins/commute/skills
```

Write `plugins/commute/plugin.json`:

```json
{
  "name": "commute",
  "description": "Travel time via Waze live traffic data",
  "containerEnvVars": [],
  "hooks": []
}
```

Write `plugins/commute/skills/SKILL.md`:
