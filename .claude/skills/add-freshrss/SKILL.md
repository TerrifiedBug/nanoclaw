...
- Adds all three variables to the container env allowlist
...
grep "FRESHRSS_URL" src/container-runner.ts | head -1
```
...
Check if the variables are already in the `allowedVars` array in `src/container-runner.ts`:

```bash
grep "FRESHRSS" src/container-runner.ts
```

If not present, add `'FRESHRSS_URL'`, `'FRESHRSS_USER'`, and `'FRESHRSS_API_KEY'` to the `allowedVars` array.
...
- **"FRESHRSS_URL not set" in container**: Check that `FRESHRSS_URL`, `FRESHRSS_USER`, and `FRESHRSS_API_KEY` are all in the `allowedVars` array in `src/container-runner.ts`, and that `.env` has the variables set.
...
2. Remove `'FRESHRSS_URL'`, `'FRESHRSS_USER'`, and `'FRESHRSS_API_KEY'` from `allowedVars` in `src/container-runner.ts`

3. Optionally remove the container skill:
```bash
rm -rf container/skills/freshrss/
```
