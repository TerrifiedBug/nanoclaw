...
The auth server serves the QR code as an SVG via HTTP on port 8900:

```bash
npx tsx src/wa-auth-business.ts &
sleep 3
...
## Step 5: Ensure Service Loads .env
...
Common issues:
- Auth expired: Re-run `npm run auth-business` (stop service first)
...
To revert to personal WhatsApp DM as main channel:

1. Remove business registration:
```bash
