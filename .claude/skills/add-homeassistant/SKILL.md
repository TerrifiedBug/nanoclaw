...
- Adds an MCP server entry to `.mcp.json` (mounted into containers automatically)
- Agents get MCP tools for HA control — no curl needed
...
cat .mcp.json 2>/dev/null | jq '.mcpServers["home-assistant"] // empty'
```
...
## Step 6: Verify Container Runner Allowlist

Check that `HA_URL` and `HA_TOKEN` are in the `allowedVars` array in `src/container-runner.ts`:

```bash
grep "'HA_URL'" src/container-runner.ts && grep "'HA_TOKEN'" src/container-runner.ts
```

Both should already be present. If not, add them to the `allowedVars` array.

## Step 7: Configure MCP Server in .mcp.json

Read the current `.mcp.json` and add (or update) the `home-assistant` MCP server entry:
...
**Important:** Preserve any existing MCP servers in the file. Merge, don't overwrite.

The `${HA_URL}` and `${HA_TOKEN}` use Claude Code's env var expansion — they reference the values from `.env` at runtime, so no secrets are stored in `.mcp.json`.

## Step 8: Verify Mount Exists

Check that `container-runner.ts` mounts `.mcp.json` into the container:

```bash
grep "mcp.json\|mcpJson" src/container-runner.ts
```

It should mount `.mcp.json` to `/workspace/.mcp.json`. If not present, add the mount after the skills directory mount.
...
- **Agent says "no MCP tools available"**: Check that `.mcp.json` has the `home-assistant` entry, HA MCP Server integration is enabled, and entities are exposed
...
2. Remove the MCP server from `.mcp.json` (delete the `home-assistant` key).
...
5. Optionally disable the MCP Server integration in HA and revoke the access token.
