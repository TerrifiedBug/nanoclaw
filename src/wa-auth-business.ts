...
const AUTH_DIR = './store/auth-business';
const PORT = 8900;
...
const server = http.createServer(async (req, res) => {
  ...
  if (authStatus === 'success') {
    res.end('<html><body style="text-align:center;font-family:sans-serif;padding:50px"><h1>Business Account Authenticated!</h1><p>WhatsApp Business is now connected. You can close this page.</p></body></html>');
    return;
  }
  ...
  res.end(`
    ...
    <body style="text-align:center;font-family:sans-serif;padding:20px">
      ...
      <p>1. Open WhatsApp Business &rarr; Settings &rarr; Linked Devices &rarr; Link a Device</p>
...
async function authenticate(): Promise<void> {
  ...
  if (state.creds.registered) {
    console.log('Business account already authenticated with WhatsApp');
    process.exit(0);
  }

  server.listen(PORT, '0.0.0.0', () => {
    console.log(`\nBusiness QR code server running at http://0.0.0.0:${PORT}`);
    ...
    sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        if (reason === DisconnectReason.loggedOut) {
          authStatus = 'failed';
          console.log('\nLogged out. Delete store/auth-business and try again.');
          server.close();
          process.exit(1);
        } else {
      ...
      if (connection === 'open') {
        authStatus = 'success';
        console.log('\nSuccessfully authenticated business account with WhatsApp!');
        console.log('  Credentials saved to store/auth-business/');
        setTimeout(() => {
          server.close();
          process.exit(0);
        }, 2000);
...
authenticate().catch((err) => {
  console.error('Business authentication failed:', err.message);
  server.close();
  process.exit(1);
});
