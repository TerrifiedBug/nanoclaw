...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'TARS';
...
Messages must start with the trigger pattern (default: `@TARS`):
- `@TARS what's the weather?` → ✅ Triggers Claude
- `@andy help me` → ✅ Triggers (case insensitive)
- `Hey @TARS` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @TARS what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@TARS what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@TARS add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@TARS remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@TARS list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@TARS remember I prefer dark mode` | Add to global memory |
...
```
User: @TARS remind me every Monday at 9am to review the weekly metrics
...
```
User: @TARS at 5pm today, send me a summary of today's emails
...
From any group:
- `@TARS list my scheduled tasks` - View tasks for this group
- `@TARS pause task [id]` - Pause a task
- `@TARS resume task [id]` - Resume a paused task
- `@TARS cancel task [id]` - Delete a task

From main channel:
- `@TARS list all tasks` - View tasks from all groups
- `@TARS schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@TARS add group "Name"` in main |
